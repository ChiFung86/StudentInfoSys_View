<template>
    <div id="container">
        <el-row>
            ...
            <el-col :span="5" :offset="1" style="line-height: 15px;">
                {{content}}
            </el-col>
        </el-row>
    </div>
    ...
    export default {
        ...
        methods:{
            ...
            checkPasswordStrength(value) {
                ...
                {   
                    if (/\d/.test(value)) mode=1; //数字
                    if (/[a-z]/.test(value)) mode=1; //小写
                    if (/[A-Z]/.test(value)) mode=1; //大写
                    if (/\W/.test(value)) mode=1; //特殊字符
                }
                if(value.length>5){
